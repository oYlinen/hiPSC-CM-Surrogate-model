...
viscosity = 0.003;  % (normalised force s um-1)
  ...
  % *****************************************************    
    ...
    am2 = H*hbT_app;
    am3 = fxbT;
    F1 = 1*myLatinHypercubeVector(22);
    F2 = 1*myLatinHypercubeVector(23);
...
global stim;
if ((time >= i_stim_Start) && (time <= i_stim_End) && (time-i_stim_Start-floor((time-i_stim_Start)/i_stim_Period)*i_stim_Period <= i_stim_PulseDuration)) && (Y(1) < -0.052 || stim==1)
